...
kernel void waterDropKernel(texture2d<float, access::read> src [[texture(0)]],
                            texture2d<float, access::write> dst [[texture(1)]],
                            constant float2 &center [[buffer(waterSimPassDropCenterIndex)]],
                            constant float &radius [[buffer(waterSimPassDropRadiusIndex)]],
                            constant float &strength [[buffer(waterSimPassDropStrengthIndex)]],
...
kernel void waterUpdateKernel(texture2d<float, access::read> src [[texture(0)]],
                              texture2d<float, access::write> dst [[texture(1)]],
                              uint2 gid [[thread_position_in_grid]]) {
    const int w = src.get_width();
    const int h = src.get_height();
...
kernel void waterNormalKernel(texture2d<float, access::read> src [[texture(0)]],
                              texture2d<float, access::write> dst [[texture(1)]],
                              uint2 gid [[thread_position_in_grid]]) {
    const int w = src.get_width();
    const int h = src.get_height();
...
kernel void waterSphereKernel(texture2d<float, access::read> src [[texture(0)]],
                              texture2d<float, access::write> dst [[texture(1)]],
                              constant float3 &oldCenter [[buffer(waterSimPassOldCenterIndex)]],
                              constant float3 &newCenter [[buffer(waterSimPassNewCenterIndex)]],
                              constant float &radius [[buffer(waterSimPassSphereRadiusIndex)]],
                              uint2 gid [[thread_position_in_grid]]) {
    const int w = src.get_width();
    const int h = src.get_height();
    if ((int)gid.x >= w || (int)gid.y >= h) {
...
vertex CausticsInOut vertexWaterCaustics(uint vid [[vertex_id]],
                                         constant float3 *positions [[buffer(waterScenePassPositionIndex)]],
                                         constant WaterSceneUniforms &u [[buffer(waterScenePassUniformIndex)]],
                                         texture2d<float> waterTex [[texture(waterScenePassWaterTextureIndex)]]) {
    constexpr sampler linClamp(address::clamp_to_edge, filter::linear);
...
fragment float4 fragmentWaterCaustics(CausticsInOut in [[stage_in]],
                                      constant WaterSceneUniforms &u [[buffer(waterScenePassUniformIndex)]]) {
...
vertex WaterScenePosOut vertexWaterPool(uint vid [[vertex_id]],
                                        constant float3 *positions [[buffer(waterScenePassPositionIndex)]],
                                        constant WaterSceneUniforms &u [[buffer(waterScenePassUniformIndex)]]) {
...
fragment float4 fragmentWaterPool(WaterScenePosOut in [[stage_in]],
                                  constant WaterSceneUniforms &u [[buffer(waterScenePassUniformIndex)]],
                                  texture2d<float> waterTex [[texture(waterScenePassWaterTextureIndex)]],
                                  texture2d<float> tiles [[texture(waterScenePassTilesTextureIndex)]],
                                  texture2d<float> causticTex [[texture(waterScenePassCausticsTextureIndex)]]) {
    constexpr sampler linClamp(address::clamp_to_edge, filter::linear);
...
vertex WaterScenePosOut vertexWaterSphere(uint vid [[vertex_id]],
                                          constant float3 *positions [[buffer(waterScenePassPositionIndex)]],
                                          constant WaterSceneUniforms &u [[buffer(waterScenePassUniformIndex)]]) {
...
fragment float4 fragmentWaterSphere(WaterScenePosOut in [[stage_in]],
                                    constant WaterSceneUniforms &u [[buffer(waterScenePassUniformIndex)]],
                                    texture2d<float> waterTex [[texture(waterScenePassWaterTextureIndex)]],
                                    texture2d<float> causticTex [[texture(waterScenePassCausticsTextureIndex)]]) {
    constexpr sampler linClamp(address::clamp_to_edge, filter::linear);
...
vertex WaterSurfaceOut vertexWaterSurface(uint vid [[vertex_id]],
                                          constant float3 *positions [[buffer(waterScenePassPositionIndex)]],
                                          constant WaterSceneUniforms &u [[buffer(waterScenePassUniformIndex)]],
                                          texture2d<float> waterTex [[texture(waterScenePassWaterTextureIndex)]]) {
    constexpr sampler linClamp(address::clamp_to_edge, filter::linear);
...
fragment float4 fragmentWaterSurfaceAbove(WaterSurfaceOut in [[stage_in]],
                                          constant WaterSceneUniforms &u [[buffer(waterScenePassUniformIndex)]],
                                          texture2d<float> waterTex [[texture(waterScenePassWaterTextureIndex)]],
                                          texture2d<float> tiles [[texture(waterScenePassTilesTextureIndex)]],
                                          texture2d<float> causticTex [[texture(waterScenePassCausticsTextureIndex)]],
                                          texturecube<float> sky [[texture(waterScenePassSkyTextureIndex)]]) {
    float4 c = shadeWaterSurface(in.worldPos, true, u, sky, tiles, waterTex, causticTex);
    return float4(water::linearFromSRGB(c.rgb), c.a);
}

fragment float4 fragmentWaterSurfaceBelow(WaterSurfaceOut in [[stage_in]],
                                          constant WaterSceneUniforms &u [[buffer(waterScenePassUniformIndex)]],
                                          texture2d<float> waterTex [[texture(waterScenePassWaterTextureIndex)]],
                                          texture2d<float> tiles [[texture(waterScenePassTilesTextureIndex)]],
                                          texture2d<float> causticTex [[texture(waterScenePassCausticsTextureIndex)]],
